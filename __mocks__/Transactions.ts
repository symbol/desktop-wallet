...
export const getFakeAggregateBondedTransaction = (data: any = undefined): AggregateTransaction => {
  return AggregateTransaction.createBonded(
    ...
    data && 'maxFee' in data ? data.maxFee : UInt64.fromUint(1234),
  )
}
...
export const getFakeTransaction = (type: number, data: any = undefined): Transaction => {
  switch (type) {
    ...
    case TransactionType.AGGREGATE_BONDED:
      return getFakeAggregateBondedTransaction(data)
    case TransactionType.MOSAIC_ALIAS:
      return getFakeMosaicAliasTransaction(data)
    case TransactionType.HASH_LOCK:
      return getFakeHashLockTransaction(data)
    case TransactionType.TRANSFER:
      return getFakeTransferTransaction(data)
  }
}
/// end-region helpers
