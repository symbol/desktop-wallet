 ...
 *
 */
import { Account, Deadline, Mosaic, MosaicId, MosaicSupplyRevocationTransaction, NetworkType, UInt64 } from 'symbol-sdk';
...
describe('transactions/ViewMosaicSupplyRevocationTransaction', () => {
    describe('use() should', () => {
        test('populate mosaic supply revocation transaction fields for source address view', () => {
            store.getters['account/currentSignerAddress'] = sourceAddress;
        ...
        test('populate mosaic supply revocation transaction fields for mosaic owner view', () => {
            store.getters['account/currentSignerAddress'] = signerPublicAccount.address;
