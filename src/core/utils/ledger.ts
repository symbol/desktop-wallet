...
export class SymbolLedger {
    ...
    async signTransaction(path: string, transferTransaction: any, networkGenerationHash: string, signer: string) {
        ...
        let payload = rawPayload.slice(0, 16) + 
            signature + 
            signer + 
    ...
    async signCosignatureTransaction(path: string, transferTransaction:AggregateTransaction, networkGenerationHash: string, signer: string) {
        const bipPath = BIPPath.fromString(path).toPathArray();
        const rawPayload = transferTransaction.serialize();
        ...
        let signature = h.slice(0, 128);
        let cosignatureSignedTransaction = new CosignatureSignedTransaction(
