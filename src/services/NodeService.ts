...
export class NodeService {
    ...
    public getKnowNodesOnly(profile: ProfileModel): NodeModel[] {
        return _.uniqBy(this.loadNodes(profile).concat(this.loadStaticNodes(profile.networkType, profile.generationHash)), 'url').filter(
    ...
    public getNodes(profile: ProfileModel, repositoryFactory: RepositoryFactory, repositoryFactoryUrl: string): Observable<NodeModel[]> {
        ...
        return nodeRepository.getNodeInfo().pipe(
            ...
            ObservableHelpers.defaultLast(
                this.createNodeModel(repositoryFactoryUrl, profile.networkType, undefined, undefined, profile.generationHash),
            ),
    ...
    private loadStaticNodes(networkType: NetworkType, generationHash?: string): NodeModel[] {
        return getNetworkConfig(generationHash).nodes.map((n) => {
    ...
    private createNodeModel(
        ...
        return new NodeModel(
            url,
            friendlyName || '',
            isDefault || !!getNetworkConfig(generationHash).nodes.find((n) => n.url === url),
            networkType,
            publicKey,
            nodePublicKey,
        );
