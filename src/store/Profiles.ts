...
export default {
  ...
  mutations: {
    setInitialized: (state: ProfileState, initialized: boolean) => { state.initialized = initialized },
    currentProfile: (state: ProfileState, currentProfile: ProfileModel) => Vue.set(state,
      'currentProfile', currentProfile),
    setAuthenticated: (state: ProfileState, isAuthenticated: boolean) => Vue.set(state,
  ...
  actions: {
    ...
    async SET_CURRENT_PROFILE({commit, dispatch}, currentProfile: ProfileModel) {
      ...
      dispatch('diagnostic/ADD_DEBUG', 'Changing current profile to ' + currentProfile.profileName,
        {root: true})
      ...
      dispatch('diagnostic/ADD_DEBUG', 'Using profile settings ' + Object.values(settings),
        {root: true})
