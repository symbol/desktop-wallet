 ...
 *
 */
import { MosaicId, SignedTransaction, MultisigAccountInfo, NetworkType, PublicAccount, Transaction } from 'symbol-sdk'
...
export class FormTransactionBase extends Vue {
  ...
  public async onSubmit() {
    ...
    await Promise.all(
      transactions.map(async (transaction) => {
        ...
        // This account is Ledger account
        else {
          this.$Notice.success({
          ...
          this.publicAccount = PublicAccount.createFromPublicKey(publicKey, networkType)
          ...
          try {
            if (options.isMultisig) {
              signedTransactions = await service.signMultisigStagedTransactions(multisigAccount, this.publicAccount)
            } else if (options.isAggregate) {
              signedTransactions = await service.signAggregateStagedTransactions(this.publicAccount)
            } else {
              signedTransactions = await service.signStagedTransactions(currentSigner)
            }
            this.resetStateAndNotifyResult(signedTransactions, networkType)
          } catch (err) {
            transport.close()
            this.$Notice.error({
  ...
  public async resetStateAndNotifyResult(signedTransactions: any, networkType: NetworkType) {
    ...
    this.$emit('close')
    const currentAccountPublicAccount = PublicAccount.createFromPublicKey(this.currentAccount.publicKey, networkType)
