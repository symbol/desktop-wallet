 ...
 *
 */
import {MosaicId, Mosaic,Deadline,UInt64,Account, AggregateTransaction,SignedTransaction,LockFundsTransaction, MultisigAccountInfo, NetworkType, PublicAccount, Transaction} from 'symbol-sdk'
        ...
        // Get account from ledger.
          ...
          if (options.isAggregate){
            if(!options.isMultisig){ 
              ...
              const signature = await symbolLedger.signTransaction(path,aggregateTx, generationHash, this.currentSigner.publicKey)
              transport.close()
              ...
              this.$store.dispatch(
                'diagnostic/ADD_DEBUG',
                `Signed aggregate transaction with account ${this.currentSigner.address.plain()} and result: ${JSON.stringify({ // addr.plain()
                  hash: signature.hash,
              ...
              } with cosignatory ${this.currentAccount.address} and result: ${JSON.stringify({
              ...
              this.$emit('close')
              const currentAccountPublicAccount = PublicAccount.createFromPublicKey(this.currentAccount.publicKey,
                networkType)
