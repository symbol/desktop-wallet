...
export default class InputPrivateKeyTs extends Vue {
  ...
  public get account(): Account {
    if (!this.privateKey || 0 !== this.privateKey.length % 2 || this.$refs.observer.fields.privateKey.invalid) {
      return null
    }
  ...
  public async createWalletByPrivate() {
    ...
    try {
      ...
      await this.$store.dispatch('account/ADD_WALLET', newAccount)
      ...
      this.$store.dispatch(
        ...
        `Profile login successful with profileName: ${this.currentProfile.profileName}`,
      )

      // redirect
      return this.$router.push({ name: 'accounts.importPrivateKey.finalize' })
    } catch (e) {
